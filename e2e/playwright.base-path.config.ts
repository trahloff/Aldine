...
export default defineConfig({
  ...
  webServer: process.env.ALDINE_BASE_PATH_URL ? undefined : {
    command: `npm run build -w apps/web && PORT=${PORT} ALDINE_BASE_PATH=/internal/aldine ALDINE_TEST_HOOKS=1 ALDINE_MCP=1 ALDINE_MCP_TOKEN=aldine-e2e-mcp DATA_DIR=$(pwd)/.data-base-path META_DIR=$(pwd)/.secrets-base-path CACHE_DIR=$(pwd)/.data-base-path/cache SENTRY_DSN= npx tsx apps/server/src/index.ts`,
    cwd: '..',
    port: PORT,
