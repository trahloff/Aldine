...
export default function OAuthConsent() {
  ...
  useEffect(() => {
    ...
    api.getOAuthClient(params.clientId, params.redirectUri)
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        const message = err instanceof OAuthApiError && err.status === 429
          ? 'Too many requests — wait a moment and reload this page.'
          : err instanceof Error ? err.message : 'Could not check the app that asked for access.';
        setClient({ status: 'error', message });
      });
    ...
    return (
      <div className="login">
        <div className="login__card consent" data-testid="oauth-error">
          ...
          <p className="consent__error">{client.message}</p>
  ...
  if (!user) return <SignInStep client={client.client} onAuthed={setUser} />;
  return <ConsentStep client={client.client} params={params} />;
}

function SignInStep({ client, onAuthed }: { client: OAuthClientInfo; onAuthed(u: AuthUser): void }) {
  ...
  return <LoginScreen providers={providers} passwordAuth={passwordAuth} onAuthed={onAuthed} heading={`Sign in to connect ${client.name}.`} />;
}

function ConsentStep({ client, params }: { client: OAuthClientInfo; params: ReturnType<typeof readAuthorizeParams> }) {
    ...
    <div className="login">
      <div className="login__card consent" data-testid="oauth-consent">
        ...
        <div className="consent__actions">
          ...
          <button className="btn btn--primary" onClick={() => decide('allow')} disabled={allowDisabled} data-testid="oauth-allow">{busy === 'allow' ? '…' : 'Allow'}</button>
        </div>
